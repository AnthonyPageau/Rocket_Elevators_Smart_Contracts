...
//     }
    ...
    function frames(uint256 steel_sheet, uint256 pulley, uint256 motor, uint256 steelcable ) public returns (uint256 numFrames) {
        require (steel_sheet >= 10 && pulley >= 2 && motor >= 2 && steelcable >= 2);
        productStruct.Name = "Frame";
        productStruct.Quantity = numFrames;
        productList.push(productStruct);
        return numFrames;
    }
    function buttons(uint256 ledlight, uint256 button) public returns (uint256 numButtons) {
